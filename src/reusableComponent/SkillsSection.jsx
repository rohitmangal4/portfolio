...
const Skills = () => {
  const skillset = [
    ...
    {
      category: "Backend",
      items: ["Node.js", "Express", "Python", "Flask", "MongoDB", "MySQL"],
    },
    {
      category: "Tools & Others",
      items: ["Git", "GitHub", "VS Code", "Figma", "Postman"],
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {skillset.map((skill, index) => (
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: index * 0.2 }}
              className="glass-card p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow"
            >
              <h3 className="text-xl md:text-2xl font-semibold text-center mb-6 text-white/80">
                {skill.category}
              </h3>
              <div className="grid min-[320px]:grid-cols-2 grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-2">
                {skill.items.map((item, skillIndex) => (
                  <motion.div
                    ...
                    className="relative"
                  >
                    <div className="px-4 py-2 rounded-lg bg-cyan-500/10 border border-cyan-500/50 text-cyan-300 text-sm md:text-base">
                      {item}
                    </div>
                  </motion.div>
                ))}
